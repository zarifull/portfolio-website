...
export function ProjectCard({ title, description, tags, icon: Icon, type, links, live }) {
  const { t } = useTranslation();
  const demoHref = links?.demo;
  const isLive = live && demoHref;
    ...
    <article className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full">
      ...
      <div className="flex gap-4">
        {isLive ? (
          ...
          <span className="flex-1 bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-400 py-4 rounded-xl font-bold flex items-center justify-center gap-2 cursor-not-allowed">
